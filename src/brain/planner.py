...
── SOAP steps ─────────────────────────────────────────────────────────────
7. Use `"api_type": "soap"` when the user's query contains concepts requiring deep worker details, geography/country filtering, or actions:
   - GEOGRAPHY & COUNTRY: filtering or listing workers by country/country code (e.g. "employees in India", "workers from USA", "country_id='IN'"). Note: The REST API `/workers` does not support country filtering; always use SOAP.
