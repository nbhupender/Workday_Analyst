"""
Workday Hire_Employee — SOAP service module with OAuth 2.0 Bearer Token Support.
...
Uses OAuth 2.0 Bearer Token authentication via custom HTTP headers instead of
...
CHANGELOG
---------
1. Converted authentication from WS-Security UsernameToken to OAuth 2.0 Bearer Token.
2. Removed legacy ISU_USERNAME and ISU_PASSWORD environment variable checks.
3. Position_Details is only built when at least one Tier-2 position detail is supplied.
4. advanced_fields (Tier 3) collides cleanly with a ValueError at request build time.
5. SOAP faults are caught specifically (zeep.exceptions.Fault) and .detail is surfaced.
"""
...
_hire_root = os.path.abspath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..")
)
load_dotenv(os.path.join(_hire_root, ".env"))

# ---------------------------------------------------------------------------
# 1. CONFIG — reads from .env; falls back to derived values only for WSDL
# ---------------------------------------------------------------------------
...
if not raw_wsdl_url or "/acme/" in raw_wsdl_url:
    if base_url:
        ...
        if path_parts:
            ...
            print(
                f"[HireSOAPService] Dynamically resolved WSDL_URL: {WSDL_URL}",
                file=sys.stderr,
            )
        else:
...
# Read the OAuth Bearer Token from environment variables (falls back to WORKDAY_API_TOKEN)
OAUTH_TOKEN = os.getenv("WORKDAY_OAUTH_TOKEN") or os.getenv("WORKDAY_API_TOKEN", "")
if not OAUTH_TOKEN:
    print(
        "[HireSOAPService] WARNING: Neither WORKDAY_OAUTH_TOKEN nor WORKDAY_API_TOKEN is set!",
        file=sys.stderr,
    )
...
# ---------------------------------------------------------------------------
# 3. HELPER — builds simple <X_Reference><ID type="...">value</ID></X_Reference>
# ---------------------------------------------------------------------------
...
def build_hire_employee_request(client: Client, args: dict[str, Any]) -> dict:
    ...
    if args.get("comment"):
        bpp_fields["Comment_Data"] = factory.Business_Process_Comment_DataType(
            Comment=args["comment"]
        )
    business_process_parameters = (
        factory.Business_Process_ParametersType(**bpp_fields)
        if bpp_fields
        else None
    )
    ...
    if worker_type and worker_id:
        ref_map = {
            "applicant": (
                "ApplicantObjectType",
                "ApplicantObjectIDType",
                "Applicant_ID",
                "Applicant_Reference",
            ),
            "former_worker": (
                "Former_WorkerObjectType",
                "Former_WorkerObjectIDType",
                "Former_Worker_ID",
                "Former_Worker_Reference",
            ),
            "student": (
                "StudentObjectType",
                "StudentObjectIDType",
            ...
            "academic_affiliate": (
                "Academic_AffiliateObjectType",
                "Academic_AffiliateObjectIDType",
                "Academic_Affiliate_ID",
                "Academic_Affiliate_Reference",
            ),
        }
        ...
        hire_data_fields[field_name] = _ref(
            factory, obj_type_name, id_type_name, worker_id, id_type
        )
        ...
        personal_data_fields = {
            "Name_Data": factory.Person_Name_DataType(
                Legal_Name_Data=factory.Legal_Name_DataType(
                    Name_Detail_Data=name_detail
                )
            )
        }
        ...
        if args.get("email_address"):
            contact_fields["Email_Address_Data"] = [
                factory.Email_Address_Information_DataType(
            ...
            contact_fields["Address_Data"] = [
                factory.Address_Information_DataType(
                    Country_Reference=_ref(
                        ...
                        "CountryObjectType",
                        "CountryObjectIDType",
                        args.get("address_country_id", "USA"),
                        "ISO_3166-1_Alpha-3_Code",
                    ),
        ...
        if contact_fields:
            personal_data_fields["Contact_Data"] = (
                factory.Contact_Information_DataType(**contact_fields)
            )
        ...
        if args.get("national_id"):
            national_id_data = factory.National_ID_DataType(
                ...
                ID_Type_Reference=_ref(
                    factory,
                    "National_ID_TypeObjectType",
                    "National_ID_TypeObjectIDType",
                    args.get("national_id_type", ""),
                ...
                Country_Reference=_ref(
                    ...
                    "CountryObjectType",
                    "CountryObjectIDType",
                    args.get("national_id_country", "USA"),
                    "ISO_3166-1_Alpha-3_Code",
                ),
            )
            personal_data_fields["Identification_Data"] = (
                factory.Person_Identification_DataType(
                    National_ID=[
                        factory.National_IDType(
                            National_ID_Data=national_id_data
                        )
                    ]
                )
            )

        personal_data = factory.Personal_Information_DataType(
            **personal_data_fields
        )
        applicant_data_fields = {"Personal_Data": personal_data}
        hire_data_fields["Applicant_Data"] = factory.Create_Applicant_DataType(
            **applicant_data_fields
        )
    else:
        raise ValueError(
    ...
    if args.get("position_id"):
        hire_data_fields["Position_Reference"] = _ref(
            factory,
            "Position_ElementObjectType",
            "Position_ElementObjectIDType",
            args["position_id"],
            "Position_ID",
        )
        ...
        hire_data_fields["Job_Requisition_Reference"] = _ref(
            factory,
            "Job_RequisitionObjectType",
            "Job_RequisitionObjectIDType",
            args["job_requisition_id"],
            "Job_Requisition_ID",
        )
    ...
    if args.get("job_profile_id"):
        position_details_fields["Job_Profile_Reference"] = _ref(
            factory,
            "Job_ProfileObjectType",
            "Job_ProfileObjectIDType",
            args["job_profile_id"],
            "Job_Profile_ID",
        )
    ...
    if args.get("time_type_id"):
        position_details_fields["Position_Time_Type_Reference"] = _ref(
            factory,
            "Position_Time_TypeObjectType",
            "Position_Time_TypeObjectIDType",
            args["time_type_id"],
    ...
    suppress_details = _POSITION_ID_SUPPRESSES_DETAILS and bool(
        args.get("position_id")
    )
    if position_details_fields and not suppress_details:
        event_data_fields["Position_Details"] = (
            factory.Position_Details_Sub_DataType(**position_details_fields)
        )
    ...
    if args.get("employee_type_id"):
        event_data_fields["Employee_Type_Reference"] = _ref(
            factory,
            "Position_Worker_TypeObjectType",
            "Position_Worker_TypeObjectIDType",
            args["employee_type_id"],
            "Employee_Type_ID",
        )
    if args.get("hire_reason_id"):
        event_data_fields["Hire_Reason_Reference"] = _ref(
            factory,
            "General_Event_SubcategoryObjectType",
            "General_Event_SubcategoryObjectIDType",
            args["hire_reason_id"],
    ...
    if event_data_fields:
        hire_data_fields["Hire_Employee_Event_Data"] = (
            factory.Hire_Employee_Event_DataType(**event_data_fields)
        )
    ...
    if args.get("compensation_package_id") or args.get("compensation_grade_id"):
        ...
        if args.get("compensation_package_id"):
            guideline_fields["Compensation_Package_Reference"] = _ref(
                factory,
                "Compensation_PackageObjectType",
                "Compensation_PackageObjectIDType",
                args["compensation_package_id"],
                "Compensation_Package_ID",
            )
        if args.get("compensation_grade_id"):
            guideline_fields["Compensation_Grade_Reference"] = _ref(
                factory,
                "Compensation_GradeObjectType",
                "Compensation_GradeObjectIDType",
                args["compensation_grade_id"],
                "Compensation_Grade_ID",
            )
        comp_fields["Compensation_Guidelines_Data"] = (
            factory.Compensation_Guidelines_DataType(**guideline_fields)
        )
    ...
    if args.get("base_pay_amount") is not None:
        pay_plan_sub = factory.Pay_Plan_Sub_DataType(
            ...
            Currency_Reference=_ref(
                factory,
                "CurrencyObjectType",
                "CurrencyObjectIDType",
                args.get("base_pay_currency_id", "USD"),
                "Currency_ID",
            ),
            Frequency_Reference=_ref(
                factory,
                "FrequencyObjectType",
                "FrequencyObjectIDType",
                args.get("base_pay_frequency_id", "Annual"),
                "Frequency_ID",
            ),
        )
        comp_fields["Pay_Plan_Data"] = factory.Pay_Plan_DataType(
            Pay_Plan_Sub_Data=[pay_plan_sub]
        )

    if comp_fields:
        hire_data_fields["Propose_Compensation_for_Hire_Sub_Process"] = (
            factory.Propose_Compensation_For_Employment_Sub_Business_ProcessType(
                Propose_Compensation_for_Employment_Data=factory.Compensation_Proposed_For_Employment_DataType(
                    **comp_fields
                )
            )
        )

    # --- Tier 3 Advanced Fields Passthrough ---
    ...
    hire_employee_data = factory.Hire_Employee_Business_Process_DataType(
        **hire_data_fields
    )

    request_kwargs = {"Hire_Employee_Data": hire_employee_data}
    if business_process_parameters is not None:
        request_kwargs["Business_Process_Parameters"] = (
            business_process_parameters
        )
...
class HireSOAPService:
    """
    Executes Workday SOAP Hire_Employee calls using OAuth 2.0 Bearer tokens.
    """

    def hire_employee(self, args: dict) -> dict:
        print(
            "[HireSOAPService] Executing Hire_Employee via SOAP (Bearer Auth)...",
            file=sys.stderr,
        )
        print(
            f"[HireSOAPService] Args received: {list(args.keys())}",
            file=sys.stderr,
        )

        # Guard: validate TIER 1 requirements before touching Workday
        has_existing = args.get("existing_worker_type") and args.get(
            "existing_worker_id"
        )
        ...
        if not (has_existing or has_new_name):
            missing.append(
                "either (existing_worker_type + existing_worker_id) or (first_name + last_name)"
            )
        if not has_position:
        ...
        try:
            ...
            elif detail is not None and not isinstance(
                detail, (str, int, float, dict, list)
            ):
                try:
        ...
        result = {
            ...
            "employee_reference": str(
                getattr(response, "Employee_Reference", None)
            ),
            "raw_response_summary": str(response)[:2000],
        }
        print(
            f"[HireSOAPService] Hire completed. Employee ref: {result['employee_reference']}",
            file=sys.stderr,
        )
        return result